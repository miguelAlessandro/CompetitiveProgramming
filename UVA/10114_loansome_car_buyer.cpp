...
int month, it;
double  pago_inicial, monto_prestado, valor_carro,
        d[N];


int main(){

    while(cin >> month >> pago_inicial >> monto_prestado >> it, month >= 0){ 
        ...
        int r = 0;
        double pago_mes = monto_prestado/month;
        double valor_depreciacion = (monto_prestado + pago_inicial)*(1 - d[0]);
        double valor_prestamo = monto_prestado;
        while(valor_depreciacion < valor_prestamo){ 
            r++;
            valor_prestamo -= pago_mes;
            valor_depreciacion = valor_depreciacion*(1 - d[r]);
        }
